...
import { Image as ImageIcon, UploadCloud, AlertCircle, Trash2, Power } from 'lucide-react';

const WebsiteBannerManagement = () => {
  ...
  const fetchBanners = async () => {
    try {
      ...
      if (data.success) {
        setBanners(data.data.filter(b => b.bannerType === 'WEBSITE'));
      }
    } catch (err) { 
      console.error(err);
    } finally {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setStatus({ type: 'success', message: 'Website Banner uploaded successfully!' });
        setFile(null);
        setPreview(null);
        setLinkUrl('');
        setAdTimer(5);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '800px', margin: '0 auto' }}>
      <div className="panel" style={{ padding: '40px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
          ...
          <div>
            <h2 style={{ fontSize: '1.8rem', color: 'var(--text-main)', marginBottom: '4px' }}>Website Banners</h2>
            <p style={{ color: 'var(--text-muted)' }}>Upload and manage image banners displayed on the public website homepage.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ border: '2px dashed var(--border-color)', borderRadius: '12px', padding: '40px', textAlign: 'center', backgroundColor: 'rgba(255,255,255,0.02)', cursor: 'pointer', position: 'relative' }}>
            ...
            {preview ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                <img src={preview} alt="Banner Preview" style={{ maxWidth: '100%', maxHeight: '200px', borderRadius: '8px', border: '1px solid var(--border-light)', objectFit: 'contain' }} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                ...
                <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
                  <strong style={{ color: 'var(--accent-yellow)'}}>Recommended Size: 600x800 pixels</strong>
                  <br />(JPG, PNG)
                </div>
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', display: 'none' }}>
            <label style={{ color: 'var(--text-muted)', fontSize: '0.9rem', fontWeight: 'bold' }}>Slider Display Timer (Seconds)</label>
            <input 
              type="number" 
              min="1"
              max="60"
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px' }}>
            <button 
              ...
              style={{ opacity: (isUploading || !file) ? 0.5 : 1, padding: '12px 32px', fontSize: '1rem', display: 'flex', gap: '8px', alignItems: 'center', backgroundColor: 'var(--accent-blue)', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
            >
              {isUploading ? 'Uploading...' : 'Upload Website Banner'}
      ...
      {/* Banner List Section */}
      <div className="panel" style={{ padding: '40px' }}>
        <h3 style={{ fontSize: '1.4rem', color: 'var(--text-main)', margin: '0 0 24px 0' }}>Manage Website Banners</h3>
        ...
        {isLoadingBanners ? (
          ...
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '32px', border: '1px solid var(--border-color)', borderRadius: '12px' }}>
            No website banners uploaded yet.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {banners.map((banner) => (
              <div key={banner._id} style={{ display: 'flex', alignItems: 'center', gap: '24px', padding: '16px', border: '1px solid var(--border-color)', borderRadius: '12px', backgroundColor: 'rgba(255,255,255,0.02)' }}>
                <img src={banner.imageUrl} alt="Banner" style={{ width: '150px', height: 'auto', borderRadius: '8px', border: '1px solid var(--border-light)', opacity: banner.isActive ? 1 : 0.4 }} />
                <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button 
                    ...
                    style={{ padding: '8px', border: 'none', borderRadius: '6px', cursor: 'pointer', backgroundColor: banner.isActive ? 'rgba(239, 68, 68, 0.1)' : 'rgba(34, 197, 94, 0.1)', color: banner.isActive ? 'var(--danger)' : 'var(--success)' }}
                  >
                    <Power size={18} />
                  </button>
                  <button 
                    ...
                    className="icon-btn" 
                    title="Delete"
                    style={{ padding: '8px', border: 'none', borderRadius: '6px', cursor: 'pointer', backgroundColor: 'rgba(239, 68, 68, 0.1)', color: 'var(--danger)' }}
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
...
export default WebsiteBannerManagement;
